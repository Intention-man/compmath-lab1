...
abstract class Reader {
  ...
  protected readMatrix(): number[][] {
    ...
    for (let i = 0; i < n; i++) {
      const row: number[] = [];
      for (let j = 0; j < n; j++) {
        const number = parseInt(numbers[oneDimIndex]);
        if (isNaN(number)) {
  ...
  protected readRightValues(): number[] {
    const n = this.n;
    const numbers = this.rawMatrixAndRightValues.split(/[\s\n]/);
    ...
    for (let i = 0; i < n; i++) {
      ...
      const rightValue = parseInt(numbers[rightValueIndex]);
      if (isNaN(rightValue)) {
  ...
  protected readEpsilon(): number {
    const result = parseFloat(this.rawEpsilon);
    if (isNaN(result)) {
